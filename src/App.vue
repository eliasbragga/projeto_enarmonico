<template>
  <div id="app">    
    ...
    <div id="sectionMaior">
      ...
      <div>
        <InputComp @adicionarNota="adicionarFunc($event)" />
      </div>
      <div v-show="mostrarResultado" id="resultado">
        <p v-bind:title="descricao">{{ nota }}</p>                  
    </div>  
      <ErrorMsg :msg="message" :Vshow="mostrar"/>
  </div>
  ...
  methods: {
    adicionarFunc(event) {
      if (event == "C") {
        ...
        this.mostrar = true
        this.message = "Não encontramos esta nota. Tente novamente"
      }

      setTimeout(() => (this.mostrar = false), 5000);
      setTimeout(() => (this.mostrarResultado = false), 15000);
    },
  },
  components: {
    InputComp,
...
#sectionMaior {
  ...
  box-shadow: 5px 5px 9px rgba(64, 64, 64, 0.407);
}
...
#resultado {
  ...
  height: 70px;
  border-radius: 10px;
  box-shadow: 5px 5px 8px rgba(77, 75, 75, 0.456);
  padding: 20px;
